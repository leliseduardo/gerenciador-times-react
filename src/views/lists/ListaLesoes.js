...
const ListaLesoes = () => {
  ...
  const add = () => {
    navigate("/cadastro-lesao");
  };

  const edit = (id) => {
    navigate(`/cadastro-lesao/${id}`);
  };
    ...
    await axios
      ...
      .then((response) => {
        if (response.data != null)
          mensagemSucesso(`Lesões ${id} excluida com sucesso`);
        mount();
      })
      .catch(() => {
        mensagemErro(`Lesao associada a jogador`);
      });
  };

  return (
    <div className="w-full h-full">
      <Navbar title="Lista de Lesões" />
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableHead>
              <TableRow>
                ...
                <TableCell align={"center"}>Tempo médio (meses)</TableCell>
            ...
            <TableBody>
              {lesions.map((lesion) => (
                ...
                >
                  ...
                  <TableCell align={"center"}>{lesion.gravidade}</TableCell>
                  <TableCell align={"center"}>{lesion.tipo}</TableCell>
                  <TableCell align={"center"}>
                    {lesion.tempoMedioDeTratamento}
                  </TableCell>
                  <TableCell align={"center"}>
